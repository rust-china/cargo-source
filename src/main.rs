...
fn main() {
  ...
  match m.get_matches_mut().subcommand() {
    Some(("list", _)) => ls(),
    Some(("use", sub_m)) => {
  ...
  Command::new("mypro")
    ...
    .subcommands([
      ...
      Command::new("use")
        .about("使用指定源")
        .arg(
          arg!(<source> "前选择源名称").required(true)
        ),
      Command::new("add")
        .about("添加源")
        .arg(
          arg!(<name> "源名称").required(true)
        )
        .arg(
          arg!(<url> "源地址").required(true)
        )
    ])
...
fn ls() {
  ...
  for (tag, url, desc) in REGISTRIES {
    println!("  {} | {} | {} ", Green.paint(pad_end(tag, 10, ' ')), url, desc)
  }
  println!("\n-------------------------------------------\n");

  println!("本地配置源：");

  let replace_with = match cargo_config.document.as_table()
    .get("source").unwrap()
    .get("crates-io").unwrap()
    .get("replace-with") {
  ...
  cargo_config.registries.iter().for_each(|(name, registry)| {
    ...
    println!("{} | {}", pad_end(&tag, 12, ' '), registry.trim_matches('\"'))
  });
